...
class mongoDBoperations :
    ...
    def __init__(self, username, password):
        ...
        try :
            ...
            self.url = "mongodb+srv://{}:{}@cluster0.qvbehlg.mongodb.net/?retryWrites=true&w=majority".format(self.username, self.password)
            #self.url = 

        except Exception as e :
            raise Exception(f"(__init__): something went wrong on initiation process.\n" + str(e))
        
    def getMonogoDBCObject(self):
        """
        This function create the client object for self purpose.
        """
        try :
            mongo_client = pymongo.MongoClient(self.url)
            return mongo_client
        except Exception as e :
            raise Exception(f"(__init__): something went wrong in intiation process\n" + str(e))
