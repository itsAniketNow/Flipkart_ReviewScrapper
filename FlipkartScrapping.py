...
class FlipkartScrapper :
    ...
    def __init__(self, executable_path, chrome_options):
        """  
        this function intializes the web browser driver 
    ...
    def waitExplicitlyforCondition(self, element_to_be_found):
        """
        This function expicitly for condition to be satisfiy
        """
        try :
            ignored_exceptions = (NoSuchElementException, StaleElementReferenceException)
            WebDriverWait(self.driver, 2, ignored_exception = ignored_exceptions).until(expected_conditions.presence_of_element_located((By.CLASS_NAME, element_to_be_found)))
            return True
        
        except Exception as e :
            return False
        
    def getCurrentWindowsUrl(self):
        """
        This function return the URL of current window
        """
        try :
            current_windows_url = self.driver.current_url
            return current_windows_url
        
        except Exception as e :
            raise Exception(f"(getCurrentWindowUrl) - something went to retriving current URL.\n" + str(e))
